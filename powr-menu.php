<?php
     ...
     */
    /*
    Plugin Name: POWr Menu
    Plugin URI: https://www.powr.io/plugins/menu
    Description: Create a stunning restaurant menu in minutes  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Menu widget/////////////////////////////////
    class Powr_Menu extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_menu',
                             __( 'POWr Menu' ),
                             array( 'description' => __( 'Menu by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-menu' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Menu.
        </p>
        <p>
    ...
    function register_powr_menu() {
      register_widget( 'Powr_Menu' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_menu' );
    //Create short codes for adding plugins anywhere:
    function powr_menu_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-menu' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-menu' label='$label'></div>";
      }else{
      	"<div class='powr-menu'></div>";
      }
    }
    add_shortcode( 'powr-menu', 'powr_menu_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_menu_menu' );
    function powr_menu_menu() {
      add_menu_page( 'POWr Menu', 'POWr Menu', 'manage_options', 'powr-menu-settings', 'powr_menu_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_menu_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/menu';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/menu';
      }
    ...
    register_activation_hook(__FILE__, 'powr_menu_plugin_activate');
      add_action('admin_init', 'powr_menu_plugin_redirect');

      function powr_menu_plugin_activate() {
      add_option('powr_menu_plugin_do_activation_redirect', true);
      }
      ...
      function powr_menu_plugin_redirect() {
      if (get_option('powr_menu_plugin_do_activation_redirect', false)) {
          delete_option('powr_menu_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-menu-settings&' );
          }
       }
      }
